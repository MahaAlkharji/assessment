...
function countElectronicsProducts(products) {
  const nop = products.filter((p) => p.category == "Electronics");
  return nop.length;
}
...
function listProductNamesByCategory(products, category) {
  const ne = products.filter((p) => p.category == category);
  return [ne[0].name, ne[1].name];
}
...
function getCheapestProduct(products) {
  const mini = Math.min([products.price]);
  const num = mini.length;
  return products[num];
}
...
function removeProductById(products, productId) {
  const idd = products.find((i) => i.id == productId);
  return idd;
}
